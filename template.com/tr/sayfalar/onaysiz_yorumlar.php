<script type="text/javascript"> 
	...
	success:function(cevap){ 
		...
		<div class="sonucu basliks">ONAY BEKLEYEN YORUMLAR <span>Toplam ( <?php echo $ysay;?> Adet ) onaysız yorum bulunuyor</span></div>
		<div class="blok">
			<form id="vidz">
			<div id="list_alani">
				<table>
					...
					<?php
						...
						$say		= $db->rowsquery("SELECT id FROM comment WHERE statu='0'");
						$sayfala 	= $say;
						$sayi	 	= 20;
						$page 		= isset($_GET["page"]) ? intval($_GET["page"]) : 1;
						$s 			= ($page-1)*$sayi;
						$sayfa		= "index.php?yorum=onaysiz&page=";
						
						$vids	= $db->tablo("SELECT * FROM comment WHERE statu='0' ORDER BY id DESC LIMIT $s,$sayi");
						foreach ($vids as $kat){
					?>
					
					<tr class="tr_alt">
						<td width="10"><input type="checkbox" name="cyorum[]" value="<?php echo $kat->id;?>"/></td> 
						<td width="100"><?php echo $kat->id;?></td> 
						<td width="420">
						...
						<td class="islem" width="62">
							...
							<a onClick="return confirm('Bu Yorumu Onaylamak istediğinize emin misiniz?')" href="islemler/yorum_onayla.php?yid=<?php echo $kat->id;?>"><img src="img/icon/onay.png" alt="" /></a>
						</td> 
					</tr>
				...
				<div class="temizle"></div>
					...
					<div class="islemler">
						...
						<input onClick="if (confirm('Seçili tüm yorumlara işlem yapılacak.Yapmak istediğinize eminmisiniz?')) {gonder();}" type="button" class="yap" value="İşlemi Gerçekleştir"/>
